...
def _create_category(category4_name, category4_name_uz, category3_name, category3_name_uz, category2_name,
    ...
    parent_category = dishservice.get_category_by_name(parent_category_name, parent_category_name_uz, 'ru')
...
def _get_or_create_category(category_name, category_name_uz, parent_category):
    if not category_name:
        return None
    category = dishservice.get_category_by_name(category_name, category_name_uz, 'ru', parent_category)
    if not category:
...
def _get_or_create_dish(product_name, product_name_uz, product_description, description_uz, image, product_price, category):
    if not product_name:
        return None
    product = dishservice.get_dish_by_name(product_name, 'ru', category)
    if not product:
        dishservice.create_dish(product_name, product_name_uz, product_description, description_uz, str(image), product_price, category.id, False)
    return product
